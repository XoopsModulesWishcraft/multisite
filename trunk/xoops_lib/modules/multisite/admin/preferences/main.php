...
if ( !is_object($xoopsUser) || !is_object($xoopsModule) || !$xoopsUser->isAdmin($xoopsModule->mid()) ) {
				...
				if(!$domain_handler->insert($domconf))
 {
   		   ...
   		   redirect_header("admin.php?fct=$fct&op=show&confcat_id=$confcat_id&domain=$domain",3,$msg);
		   xoops_cp_footer();
		   exit;
	}
 ...
 xoops_cp_footer();
    ...
    if ($op == 'show') {
        ...
        $form->addElement(new XoopsFormButton('', 'button', _GO, 'submit'));
            xoops_cp_header();
	adminMenu(0);
        echo '<a href="admin.php?fct=preferences">'. _MD_AM_PREFMAIN .'</a>&nbsp;<span style="font-weight:bold;">&raquo;&raquo;</span>&nbsp;'.constant($confcat->getVar('confcat_name')).'&nbsp;&raquo;&raquo;&nbsp;'.$obj_domain->getVar('dom_value').'<br /><br />';
        $form->display();
       footer_adminMenu();
 xoops_cp_footer();
        exit();
