...
function xoops_domain_list($op, $fct)
{
	...
	foreach ($configs as $config)
	{
		...
		if (count($configs_a)>0)
			$language[$ii] = new XoopsFormSelect(_MD_AM_LANGUAGE, "languages[$ii]", $configs_a[0]->getVar('dom_value'), 1 , false);
		else
		...
		$critera_y->add(new Criteria('dom_name', 'domain_themes')) ;
		$configs_b = $config_handler->getDomains($critera_y);
		
		if (count($configs_b)>0)
			$themes[$ii] = new XoopsFormSelect(_MD_AM_THEME, "themes[$ii]", $configs_b[0]->getVar('dom_value'), 1, false);
		else
			$themes[$ii] = new XoopsFormSelect(_MD_AM_THEME, "themes[$ii]", 0, 1, false);		
		$themes[$ii]->addOptionArray($xl->getThemesList());
		
		$critera_z = new CriteriaCompo(new Criteria('dom_pid', $yy));
		$critera_z->add(new Criteria('dom_name', 'start_module')) ;
		$configs_c = $config_handler->getDomains($critera_z);
	
		if (count($configs_c)>0)
			$startmodule[$ii] = new XoopsFormSelect(_MD_AM_START_MODULE, "start_module[$ii]", $configs_c[0]->getVar('dom_value'), 1 , false);
		else
		...
		$critera_z = new CriteriaCompo(new Criteria('dom_pid', $yy));
		$critera_z->add(new Criteria('dom_name', 'page_title')) ;
		$configs_c = $config_handler->getDomains($critera_z);
