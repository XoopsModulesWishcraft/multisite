...
if ( !is_object($xoopsUser) || !is_object($xoopsModule) || !$xoopsUser->isAdmin($xoopsModule->mid()) ) {
				...
				if(!$domain_handler->insert($domconf))
 {
    ...
    if ($op == 'list') {
	    ...
	    $sform = new XoopsThemeForm(_AM_SELECT_DOMAIN, array("domain","fct","op", "confcat_id"),  "admin.php?fct=$fct&op=$op&confcat_id=$confcat_id&domain=$domain");
    	$sform->setExtra('enctype="multipart/form-data"');
		$sform->addElement(new XoopsFormSelectDomains(_AM_DOMAINS, "domain", !empty($domain)?$domain:XOOPS_URL, 1, false));
		$sform->addElement(new XoopsFormButton('', '', _SUBMIT, 'submit'));
		$sform->display();
		
        echo '<h4>'._MD_AM_SITEPREF.'</h4><ul>';
        for ($i = 0; $i < $catcount; $i++) {
            echo '<li>'.constant($confcats[$i]->getVar('confcat_name')).' [<a href="admin.php?fct=preferences&amp;op=show&amp;confcat_id='.$confcats[$i]->getVar('confcat_id')."&amp;domain=$domain\">"._EDIT."</a>]</li>";
        }
        echo '</ul>';
       footer_adminMenu();
 xoops_cp_footer();
    ...
    if ($op == 'show') {
        ...
        $form->addElement(new XoopsFormButton('', 'button', _GO, 'submit'));
            xoops_cp_header();
	adminMenu(XOOPS_MULTISITE_PREF);
        echo '<a href="'.XOOPS_URL.'/modules/multisite/admin.php?fct='.$fct.'&domain='.$domain.'">'. _MD_AM_PREFMAIN .'</a>&nbsp;<span style="font-weight:bold;">&raquo;&raquo;</span>&nbsp;'.constant($confcat->getVar('confcat_name')).'&nbsp;&raquo;&raquo;&nbsp;'.$obj_domain->getVar('dom_value').'<br /><br />';
		$form->display();
		footer_adminMenu();
		xoops_cp_footer();
        exit();
    ...
    if ($op == 'save') {
        ...
        if (!$GLOBALS['xoopsSecurity']->check()) {
            redirect_header("admin.php?fct=preferences&domain=$domain", 3, implode('<br />', $GLOBALS['xoopsSecurity']->getErrors()));
        }
        ...
        if (isset($redirect) && $redirect != '') {
            ...
            redirect_header("admin.php?fct=preferences&domain=$domain", 2, _MD_AM_DBUPDATED);
        }
    }
}
